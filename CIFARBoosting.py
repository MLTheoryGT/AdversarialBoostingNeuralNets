...
def test_ensemble(path, attacks, num_wl, maxSamples=750000, numsamples_train=200, numsamples_val=1500, attack_eps_ensemble=epsilons, gradOptWeights=False, attack_iters=20, restarts=1, train_eps_nn=8):
    ensemble = testEnsemble(path, [attack], num_wl, numsamples_train=numsamples_train, numsamples_val=numsamples_val, attack_eps_ensemble=attack_eps_ensemble, gradOptWeights=gradOptWeights, attack_iters=attack_iters, restarts=restarts)
...
if __name__ == '__main__':
    ...
    path = f'./models/{args.maxSamples}Eps{args.train_eps_nn}'
    ...
    if args.test:
        ensemble = test_ensemble(path, attacks, args.num_wl, maxSamples=args.maxSamples,numsamples_train=args.numsamples_train,
        numsamples_val=args.numsamples_val, attack_iters=args.attack_iters, restarts=args.testing_restarts, train_eps_nn=args.train_eps_nn)
