...
import gc
import sys
from utils import cifar10_mean, cifar10_std, cifar100_mean, cifar100_std
...
def SchapireWongMulticlassBoosting(weakLearnerType, numLearners, dataset, maxSamples, alphaTol=1e-5, attack_eps_nn=[], attack_eps_ensemble=[],train_eps_nn=0.127, adv_train_prefix=0, val_attacks=[], predictionWeights=False, batch_size=200, model_base=PreActResNet18, attack_iters=20, val_restarts=1, lr_max=0.2):    
    ...
    def gcLoop():
        ...
        train_sampler = BoostingSampler(train_ds_index, C_tp)
        ...
        if t < adv_train_prefix:
            ...
            h_i.fit(train_loader, test_loader, C_t, adv_train=False, val_attacks=val_attacks, maxSample=maxSamples, predictionWeights=predictionWeights, attack_iters=attack_iters, restarts=val_restarts, lr_max=lr_max, dataset_name=dataset_name)
